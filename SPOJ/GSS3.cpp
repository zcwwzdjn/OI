...
void ChangeOnPoint(int x, int v, Node *pos, int pos_l, int pos_r) {
	if (pos_l + 1 == pos_r) {
		pos->info = Info(v, v, v, v);
		return;
	}
	int mid = (pos_l + pos_r) >> 1;
	if (x < mid)
		ChangeOnPoint(x, v, pos->son[0], pos_l, mid);
	else
		ChangeOnPoint(x, v, pos->son[1], mid, pos_r);
	pos->update();
}
...
int main() {
	...
	for (int t, l, r; m --; ) {
		scanf("%d%d%d", &t, &l, &r), -- l;
		if (t == 0) ChangeOnPoint(l, r, root, 0, n);
