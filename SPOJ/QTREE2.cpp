...
Node *BuildTree(int l, int r) {
	Node *res = node_idx ++;
	res->l = l, res->r = r;
	if (l == r) res->v = seq[buf[dep[buf[0]] - l]], res->t = buf[dep[buf[0]] - l];
	else {
		int mid = (l + r) >> 1;
...
int Ask(int u, int p) {
	...
	for ( ; root[u] != root[p]; u = jump[u])
		res += AskOnRange(0, dep[u], root[u]);
	if (dep[p] + 1 <= dep[u])
		res += AskOnRange(dep[p] + 1, dep[u], root[u]);
	return res;
}

int FindPoint(int x, Node *pos) {
	if (pos->l == pos->r) return pos->t;
	int mid = (pos->l + pos->r) >> 1;
	if (x <= mid) return FindPoint(x, pos->son[0]);
	else return FindPoint(x, pos->son[1]);
}

int Find(int u, int d) {
	while (root[u]->l > d) u = jump[u];
	return FindPoint(d, root[u]);		
}
	...
	while (TST --) {
		...
		while (scanf("%s", cmd) != EOF && cmd[1] != 'O') {
			...
			if (cmd[0] == 'K') {
				scanf("%d", &k);
				if (dep[a] - dep[p] + 1 < k) {
					k = dep[b] - dep[p] + 1 - (k - (dep[a] - dep[p] + 1));
					a = b;
				}
				printf("%d\n", Find(a, dep[a] - k + 1) + 1);
			}
